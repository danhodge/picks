...
export const Scoreboard = (props: ScoreboardProps) => {
  ...
  const rows = Array.from(props.participants.values()).sort(sorter).map((val: Participant, idx: number) => {
    return <>
      <div className={rowBg[idx % 2] + " col-span-4 grid grid-cols-9"}>
      ...
      <div className={rowBg[idx % 2] + " col-span-2 text-sm p-1"}>{val.score.pointsWon}</div>
      <div className={rowBg[idx % 2] + " col-span-2 text-sm"}>{val.score.pointsLost}</div>
      <div className={rowBg[idx % 2] + " col-span-2 text-sm"}>{val.score.pointsRemaining}</div>
      <div className={rowBg[idx % 2] + " col-span-2 text-sm"}>{val.score.scoringAvg?.toFixed(2)}</div>
    </>
  });
